  ...
  return (
    <div className="flex-1 grid grid-cols-2 gap-4 h-[500px]">
      {promotions.map((promo, index) => (
        <div key={index} className={`${promo.bgColor} rounded-lg p-4 flex flex-col`}>
          ...
          <div className="grid grid-cols-2 gap-4 flex-1">
            {promo.items.map((item, idx) => (
              <div key={idx} className="bg-white rounded-lg p-2 shadow-sm flex flex-col">
                <img 
                  src={item.image}
                  alt={item.title}
                ...
                <div className="text-red-500 font-bold mt-auto">{item.price}</div>
              </div>
            ))}
          </div>
        </div>
